...
export const DEFAULT_RULES={profileName:'京都府参考運用（初期値）',enabled:true,minFemaleMatingMonths:18,maxFemaleMatingYears:6,maxLifetimeBirths:6,recordRetentionYears:5,ledgerColumns:{matingAge:true,lifetimeBirths:true,dueDate:true,motherCondition:true,cesarean:true,memo:true},updatedAt:''};
export function normalizeRules(value){const source=value&&typeof value==='object'?value:{};return {...DEFAULT_RULES,...source,enabled:source.enabled!==false,minFemaleMatingMonths:Math.max(0,Number(source.minFemaleMatingMonths??DEFAULT_RULES.minFemaleMatingMonths)),maxFemaleMatingYears:Math.max(0,Number(source.maxFemaleMatingYears??DEFAULT_RULES.maxFemaleMatingYears)),maxLifetimeBirths:Math.max(0,Number(source.maxLifetimeBirths??DEFAULT_RULES.maxLifetimeBirths)),recordRetentionYears:Math.max(1,Number(source.recordRetentionYears??DEFAULT_RULES.recordRetentionYears)),ledgerColumns:{...DEFAULT_RULES.ledgerColumns,...source.ledgerColumns}};}
export function breedingRuleWarnings({mother,matingDate,pastBirthCount},value){const rules=normalizeRules(value);if(!rules.enabled||!mother||!matingDate)return[];const age=ageOn(mother.birthDate,matingDate),months=age.years*12+age.months,warnings=[];if(months<rules.minFemaleMatingMonths)warnings.push(`母犬の交配時年齢が設定下限（${rules.minFemaleMatingMonths}か月）未満です`);if(age.years>=rules.maxFemaleMatingYears)warnings.push(`母犬の交配時年齢が設定上限（${rules.maxFemaleMatingYears}歳）以上です`);if(Number(pastBirthCount)>=rules.maxLifetimeBirths)warnings.push(`過去の出産回数が設定上限（${rules.maxLifetimeBirths}回）以上です`);return warnings;}
...
export function validateBackup(value){
  ...
  const data=value.data; if(!data||![1,2,3,4,5,6,7].includes(data.schemaVersion))errors.push('スキーマバージョンが未対応です');
